...
def get_args_parser(subparsers):
    ...
    subparsers.add_argument('--csc', action='store_true', default=False,
                            help='Wrap LoRA wnew layers with CSC quantizer.')
    subparsers.add_argument('--gamma_comp', type=float, default=0.0,
                            help='CSC compression coefficient (default 0; set ~1e-3 with --csc).')
    subparsers.add_argument('--importance', type=str, default='mas', choices=['mas', 'bd'],
                            help='Importance signal for the omega regularizer: '
                                 '`mas` (4-sample empirical Fisher, upstream default) '
                                 'or `bd` (CSC bit-depth, requires --csc).')
    ...
    subparsers.add_argument('--all-metrics', dest='all_metrics', action='store_true')
